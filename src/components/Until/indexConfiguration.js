...
class IndexConfiguration extends PureComponent{
  constructor(props){
    ...
    this.state={
      ...
      mockData:[], // 所有指标
      dataSource:props.indexConfigData, // 左侧展示待选指标
      targetKeys:props.indexConfigData, // 右侧展示已选指标
  ...
  componentDidUpdate(pevProps,pevState){
    ...
    if(!pevState.visible&&visible){
      this.setState({
        targetKeys:saveData,
        mockData:this.props.indexConfigData, // 所有指标
        dataSource:this.props.indexConfigData, // 左侧展示待选指标
      })
    }
  }
  ...
  componentWillUnmount(){
    ...
    dispatch({
      type:"IndexConfigModels/getIndexConfig",
      payload:[]
    })
  }
  ...
  handleOk=()=>{
    const {dispatch,callBcakOk}=this.props;
    const {targetKeys}=this.state;
