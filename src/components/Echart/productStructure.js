...
import styles from './productStructure.less';
import EchartFontSize from './echartFontSize';

class ProductStructure extends PureComponent {
  ...
  constructor(props) {
    ...
    this.productStructureRef = React.createRef(); // 产品结构图ref
  }
  ...
  initEchart() {
    const { data } = this.props;
    const productStructureData = data;
    const showCont = "本月累计";
    const {unit, chart} = productStructureData.data[0];
    const chartPie = chart;
    const numberArr = this.handleChartData(chartPie);
    const legendData = [];
    const {isMinus,isPercentage} = productStructureData;// 是否是百分比指标（1表示是百分比指标，0表示不是）-如果是，饼图改为柱状图显示
    ...
    const option = {
      ...
      color: ["#DE65AD", "#F97373", "#E0E7EF", "#56D5E4", "#5CACE0"],
      series: [
        {
          type: "pie",
    ...
    const myChart = echarts.init(this.productStructureRef.current); // 初始化当日趋势图所需dom
    // 使用刚指定的配置项和数据显示图表
    myChart.clear();
    myChart.resize();
  ...
  download(e){
    e.stopPropagation();
    DownloadFile(this.jsonHandle(),"productStructure");
  }
  ...
  render() {
    ...
    return (
      <div id="productStructure" className={styles.productStructureCss}>
        <div
          ref={this.productStructureRef}
