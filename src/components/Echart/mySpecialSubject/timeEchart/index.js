...
class MySpecialTimeEchart extends PureComponent{
  ...
  createChart = (data) => {
    ...
    const {color,stack,timeEchartArea} = this.props;
  ...
  jsonHandle=()=>{
    ...
    const unitCopy = chart[0].unit;
    const tableValue = chartX.map((item,index)=>([item,chart[0].value[index]]));
    const table={
      title: [
        ["账期",chart[0].name]
      ],
...
export default MySpecialTimeEchart;
