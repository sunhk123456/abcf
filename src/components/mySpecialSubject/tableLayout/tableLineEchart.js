...
import MySpecialTimeEchartTable from './echartTable';
...
class MySpecialLineEchart extends Component {
  ...
  download = (e) => {
    ...
    e.stopPropagation();
    DownloadFile(this.jsonHandle(),flag==="01" ? "timeEchart1":null);
  };

  jsonHandle=()=>{
    const {downloadData, chartData,newDownload} = this.props;
    ...
    let conditionValue=[];
    if(newDownload){
      conditionValue=Object.assign([],downloadData.conditionValue)
    }else {
  ...
  render() {
    const {downloadData,chartData,background}=this.props;
    ...
    return (
      <div id="timeEchart1" className={styles.page} style={{background:background||"#fff"}}>
        <div ref={this.chartDom} style={{display: flag==="01" ? "block" : "none" }} className={styles.chart} />
        <div style={{display: flag !=="01" ? "block" : "none" }} className={styles.tableWrapper}><MySpecialTimeEchartTable tableData={chartData} /></div>
        {downloadData?(
...
export default MySpecialLineEchart;
