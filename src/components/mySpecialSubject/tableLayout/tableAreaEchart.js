...
import styles from './tableAreaEchart.less';
...
class TableAreaEchart extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (!isEqual(chartData,prevState.chartData)) {
      this.createChart(chartData);
    }else if(chartData && chartData === prevState.chartData && !isEqual(queryCondition,prevProps.queryCondition)){
      this.createChart(chartData);
    }
  }
  ...
  createChart=(chartData)=>{
    ...
    const {queryCondition,queryCondition:{selectPro,selectCity},titlePosition}=this.props;
    console.log("queryCondition");
    console.log(queryCondition);
    let provinceName = chartX;
    const {proName } = selectPro;
    const {cityId,cityName} = selectCity;
    if(proName==="北十省" && cityId === "-1"){
      provinceName=["北京","天津","辽宁","内蒙古","河南","吉林","山西","河北","黑龙江","山东"]
    }else if(proName === "南二十一省" && cityId === "-1"){
      provinceName=["上海","江苏","浙江","安徽","福建","江西","湖北","湖南","广东","广西","海南","重庆","四川","贵州","云南","西藏","陕西","甘肃","青海","宁夏","新疆"]
    }else if(proName === "全国" && cityId === "-1"){
      provinceName=chartX;
    }else if(cityId === "-1"){
        ...
        itemStyle:{
          color(params){
            ...
            provinceName.forEach((item) =>{
              if(item === params.name || item.search(params.name) !== -1){
  ...
  render() {
    const {downloadData,cockpit,widthInfo}=this.props;
    ...
    return (
      <div id="areaEchart2" className={styles.page}>
        {/* 当cockpit为false或图表占1/3框时，下拉列表距离顶部45px */}
        <div id="regionSituation" className={styles.regionSituationCss} style={{top:!cockpit || widthInfo === '2' ?'45px':'10px'}}>
        ...
        {downloadData?(
          <div className={styles.downLoad} onClick={(e)=>this.download(e)} style={{top:cockpit?"10px":"45px"}}>
...
export default TableAreaEchart;
