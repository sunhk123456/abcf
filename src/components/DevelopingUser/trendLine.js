...
class TrendLine extends PureComponent {
  ...
  initLine(data) {
    if(data.length !== 0){
      const data1 = data.chart[0].map((el)=>{
        if(el === "-"){
          return "-"
        }
        return parseFloat(el.replace(/,/g, ''));
      });
      const data2 = data.chart[1].map((el)=>{
        if(el === "-"){
          return "-"
        }
        return parseFloat(el.replace(/,/g, ''));
      });
      const data3 = data.chart[2].map((el)=>{
        if(el === "-"){
          return "-"
        }
        return parseFloat(el.replace(/,/g, ''));
  ...
  jsonHandle(){
    const {indexDate,trendLineData,selectPro,selectCity,indexType,indexType1} = this.props;
    ...
    chartX.forEach((item,index)=>{
      tableValue.push([item,chart[0][index],chart[1][index],chart[2][index]])
    })
    const condition = {
      name:  "用户发展真实性预测",
      value: [
    ...
    const  table = {
      title: [
        ["日期", `${example[0]}(${unit[0]})`, `${example[1]}(${unit[1]})`, `${example[2]}(${unit[2]})`]
      ],
      value: [
        ...tableValue
      ]
  ...
  render() {
    ...
    return (
      <div id="trendLineDownload" className={yangshi.lineOuter} style={{display:comStatus.lineStatus,height:barShow==='true'?'44%':'88%',top:barShow==='true'?'2%':'0'}}>
        ...
        <div className={yangshi.echart} ref={this.trendLineRef} />
      </div>
    );
  }
