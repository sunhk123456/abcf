...
class MonthSpecialChange extends PureComponent {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      rangeDate:moment(moment().subtract(1,'months')).format(monthFormat), // 暂存已选日期,默认今日
      selIndex:[],// 表格已选数据
  ...
  componentWillMount(){
    ...
    dispatch({
      ...
      payload: { dateType:'2',mark:"special" }
    });
      ...
      <PageHeaderWrapper>
        <Fragment>
          <Card bordered={false}>
            <Row>
              <Col md={24}>
                <div className={yangshi.mainTitle}>月专题账期列表</div>
...
export default MonthSpecialChange;
